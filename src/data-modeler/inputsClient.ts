...
export const inputsClient = (classes: any, onClickReset: Function): IInput<any>[] => [
    {
        name: 'name',
        label: 'Name',
...
export const filtersClient: Array<keyof IClient> = ["name", 'id', 'active', 'email', 'lastName', 'secondLastName']

export const headersClient = (buttons: IButtonTable[]): IHeaderTable[] => [
    {align: "center", size: 1, type: EnumColumnTable.Normal, field: 'id', headerName: 'Id'},
    {
        align: "center",
        size: 3,
        type: EnumColumnTable.Handler,
        field: ({name, lastName, secondLastName}: IClient) => `${name} ${lastName} ${secondLastName}`,
        headerName: 'Name'
    },
